var textureGalaxyGeneratorFragmentShader = "\
		varying vec2 vUv; \
		uniform int index; \
		uniform float randomFloatX; \
		uniform float randomFloatY; \
		\
		...
		float random5(vec3 co) { \
			return fract(sin(dot(co.xyz ,vec3(randomFloatX, randomFloatY, 1.23456))) * 43758.5453); \
		} \
		\
		...
		void main() { \
			...
			\
			gl_FragColor = vec4((color.x+randomRed)*randomRed, (color.y+randomGreen)*randomGreen, (color.z+randomBlue)*randomBlue, color.x*color.y*color.z); \
		}\
	";