...
Game.util.clamp = function(number, from, to) {
	/*
	 * The Math.min() function returns the smallest of zero or more numbers. The
...
Game.util.random = function(seed) {
	 ...
	 * that is, base^exponent. Math.pow(base, exponent)
	 */
...
window.P = Game.util.makeSpecifiedArray1D(N, function() {
	return Game.util.randomInt(0, N - 1)
}, new Uint32Array(N));
/**
 * To do
 */
...
/**
 * The camera Rotating the camera id mouse button is down. handels the rotation
 * and zooming of the camera.
 * 
 * @param camera
 * @param document
 * @param startRadius
 ...
 * 
 */
Game.util.EditorCamera = function(camera, document, startRadius,
		cameraStartPos, originObject) {
	/**
	 * this
	 * 
	 ...
	 * lexically scoped (it is set to the this value of the enclosing execution
	 * context).
	 * 
	 ...
	 * undefined. It can also be set to any value, such as null or 42 or "I am
	 * not this".
	 * 
	 * Reade more:
	...
	this.cameraStartPos = cameraStartPos
			|| new THREE.Vector3(Math.PI / 4, -Math.PI / 4);
	...
	var addEventListeners = function() {
		...
		document.addEventListener('mousemove', function(event) {
			...
			if (editorCamera.mouseDown == true) {
				var diff = mousePos.clone().sub(editorCamera.mouseClickPos)
						.multiplyScalar(1 / 250);
				editorCamera.cameraPos = editorCamera.cameraStartPos.clone()
						.add(diff);
		...
		document.addEventListener('mousewheel',
				function(event) {
					...
					if (editorCamera.getScaledRadius(editorCamera.radius
							- delta) >= 0) {
	...
	this.getScaledRadius = function(radius) {
		/*
		 * Math.exp() The Math.exp() function returns e^x, where x is the
		 * argument, and e is Euler's number (also known as Napier's constant),
		 * the base of the natural logarithms. Zoom - if we don't want zoom
		 * capabillitys we can remove the variables belove. Math.exp(radius) -
		 * Math.exp(this.startExp)
		 */
	...
	this.zoomCamera = function() {
		this.camera.position.normalize().multiplyScalar(
				this.getScaledRadius(this.radius));
	}
	/**
...
/**
 * This listener function adds a listener to the window. changes the camera
 * aspect ration and the window size.
 */
Game.util.addResizeListener = function() {
	window.addEventListener('resize', function() {
		...
		/*
		 * .aspect Camera frustum aspect ratio, window width divided by window
		 * height.
		 */
		camera.aspect = window.innerWidth / window.innerHeight;
		camera2.aspect = window.innerWidth / window.innerHeight;
		/*
		 * .updateProjectionMatrix () Updates the camera projection matrix. Must
		 * be called after change of parameters.
		 */
...
Game.util.computeGeometry = function(geometry) {
	geometry.makeGroups();
	/**
	 * .computeVertexNormals () Computes vertex normals by averaging face
	 * normals. Face normals must be existing / computed beforehand.
	 */
	...
	/**
	 * .computeBoundingSphere () Computes bounding sphere of the geometry,
	 * updating Geometry.boundingSphere attribute.
	 * 
	 * Neither bounding boxes or bounding spheres are computed by default. They
	 * need to be explicitly computed, otherwise they are null.
	 */
	geometry.computeBoundingSphere();
	/**
	 * .computeBoundingBox () Computes bounding box of the geometry, updating
	 * Geometry.boundingBox attribute.
	 */
	geometry.computeBoundingBox();
	/**
	 * .computeLineDistances () Compute distances between vertices for Line
	 * geometries.
	 */
	geometry.computeLineDistances();

	/**
	 * .verticesNeedUpdate Set to true if the vertices array has been updated.
	 */
	geometry.verticesNeedUpdate = true;
	/**
	 * .elementsNeedUpdate Set to true if the faces array has been updated.
	 */
	geometry.elementsNeedUpdate = true;
	/**
	 * .uvsNeedUpdate Set to true if the uvs array has been updated.
	 */
	geometry.uvsNeedUpdate = true;
	/**
	 * .normalsNeedUpdate Set to true if the normals array has been updated.
	 */
	...
	/**
	 * .colorsNeedUpdate Set to true if the colors array has been updated.
	 */
	geometry.colorsNeedUpdate = true;
	/**
	 * .lineDistancesNeedUpdate Set to true if the linedistances array has been
	 * updated.
	 */
...
/**
 * Interpolation At the moment we are not using this Interpolation we are using
 * a webGL based one. But the idée is the same. Interpolation is very
 * interesting.
 * 
 * So we are going to sample each texel coordinate in the field. But of course,
 * the coordinates on the sphere is probably not going to align perfectly with a
 * coordinate in the field, so we need to interpolate between the nearest
 * neighbours. There are many ways to interpolate.
 */

/**
 * Trilinear interpolation
 * 
 * This is prettier, but it is easy to see the squarish/linear look. It is
 * slower than nearest neighbour because 4 neigbour values are used in 7 linear
 * equations, but it still is a very fast way to get a smooth look.
 */
Game.util.trilinearInterpolation = function(coordFloat, scalarField,
		interpolation) {
	...
	var c00 = interpolation(scalarField(coord0.x, coord0.y, coord0.z),
			scalarField(coord1.x, coord0.y, coord0.z), xd);
	var c10 = interpolation(scalarField(coord0.x, coord1.y, coord0.z),
			scalarField(coord1.x, coord1.y, coord0.z), xd);
	var c01 = interpolation(scalarField(coord0.x, coord0.y, coord1.z),
			scalarField(coord1.x, coord0.y, coord1.z), xd);
	var c11 = interpolation(scalarField(coord0.x, coord1.y, coord1.z),
			scalarField(coord1.x, coord1.y, coord1.z), xd);
...
Game.util.nearestNeighbour = function(coordFloat, scalarField) {
	return scalarField(Math.floor(coordFloat.x), Math.floor(coordFloat.y), Math
			.floor(coordFloat.z));
}
 ...
 * 
 * This is a very good compromise. The result is almost as smooth as the
 * tricubic, and almost as fast as the trilinear. In most cases, it is
 * indistuingishable from tricubic interpolation. This is what we are going to
 * use. The algorithm is basically the same as trilinear interpolation, but with
 * a cosine function instead of the linear one.
 */
Game.util.tricosineInterpolation = function(coordFloat, scalarField) {
	...
	return Game.util.trilinearInterpolation(coordFloat, scalarField,
			interpolation);
}
 ...
 * of bump mapping. It is used to add details without using more polygons. A
 * common use of this technique is to greatly enhance the appearance and details
 * of a low polygon model by generating a normal map from a high polygon model
 * or height map.
 * 
...
Game.util.heightToNormalMap = function(map, intensity) {
	...
	var getHeight = function(x, y) {
		...
		return (map.image.data[(y * width + x) * 4 + 0] / 255
				+ map.image.data[(y * width + x) * 4 + 1] / 255 + map.image.data[(y
				* width + x) * 4 + 2] / 255)
				/ 3 * intensity;
	}

	var normalMap = THREE.ImageUtils.generateDataTexture(width, height,
